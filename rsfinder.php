	...
	public function onAfterRender() {
		...
		if ($app->getName() != 'site') {
			if (isset($search) && strlen($search) > 1 && $user->get('id')) 	{	
				...
				// Hikashop products
				if ($this->params->get('searchHikashop', 0) && $index < $limit) {
					$db->setQuery("SELECT product_id as value, `product_name` as text FROM #__hikashop_product WHERE product_name LIKE '%".$db->escape($search)."%' LIMIT ".$limit);
					$results = $db->loadObjectList();
					
					if (!empty($results)) {
						foreach ($results as $result) {
							if ($index < $limit) {
								$output .= '<li><a href="index.php?option=com_hikashop&ctrl=product&task=edit&cid[]='.$result->value.'" id="result_'.$index.'" class="rsInactive">'.$this->escape($result->text).' <em>'.JText::_('PLG_SYSTEM_RSFINDER_EDIT_HIKA').'</em></a></li>'."\n";
							}
							$index ++;
						}
					}
